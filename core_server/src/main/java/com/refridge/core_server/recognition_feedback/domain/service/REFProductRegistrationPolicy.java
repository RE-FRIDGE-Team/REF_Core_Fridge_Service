...
/**
 * Product 자동 등록 조건을 판단하는 도메인 서비스입니다.
 * <p>
 * 피드백 집계 결과를 바탕으로 "이 제품을 Product로 등록할 만큼 충분히 검증됐는가"를
 * 결정하는 비즈니스 정책을 단일 책임으로 관리합니다.
 *
 * <h3>정책: 가중 점수제 + 최소 긍정수 하한</h3>
 * <pre>
 *   score = 긍정수 * POSITIVE_WEIGHT(+1) + 부정수 * NEGATIVE_WEIGHT(-2)
 *   등록 조건: 긍정수 >= MIN_POSITIVE(3) AND score >= SCORE_THRESHOLD(5)
 * </pre>
 *
 * 케이스별 동작:
 * <pre>
 *   긍정 3 / 부정 0 → score= 3 → MIN_POSITIVE ✅, score ❌ → 미등록
 *   긍정 5 / 부정 0 → score= 5 → MIN_POSITIVE ✅, score ✅ → 등록
 *   긍정 5 / 부정 1 → score= 3 → MIN_POSITIVE ✅, score ❌ → 미등록 (부정 패널티)
 ...
 * <h3>설계 의도</h3>
 * <ul>
 *   <li>긍정만 받는 제품: 5번째부터 등록 (빠른 파이프라인 강화)</li>
